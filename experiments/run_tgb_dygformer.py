#!/usr/bin/env python
"""DyGFormer-lite baseline, and DyGFormer-lite + GraphEagleVision.

A Transformer-over-interaction-history base model (a stronger alternative to
TGN), with the same fusion modes (concat/additive/gated/attn/film) and the
same alternative couplings (score_ensemble, aux) as run_tgb_tgn.py.
"""
...
def main():
    ...
    ap.add_argument("--use_cooccur", type=int, default=0,
                    help="1 to enable the neighbour-co-occurrence trick (pair-API, slower eval)")
    ap.add_argument("--dropout", type=float, default=0.1)
    # GEV (empty indicators -> pure DyGFormer baseline)
    ap.add_argument("--gev_indicators", default="degree,core",
                    help='comma list; empty string "" disables GEV (pure DyGFormer-lite)')
    ap.add_argument("--coupling", default="fusion", choices=["fusion", "score_ensemble", "aux"],
                    help="fusion = late fuse z & h_struct via --fusion; score_ensemble = "
                         "alpha*score_BASE + (1-alpha)*score_GEV (learnable alpha); "
                         "aux = late fusion + auxiliary head making BASE predict the indicator values")
    ...
    print(f"[setup] dataset={args.dataset} model={'DyGF+GEV('+args.gev_indicators+')' if use_gev else 'DyGF'} "
    ...
    # --- models ---
    base = DyGFormerLite(N, raw_msg_dim, embedding_dim=args.embedding_dim,
                         time_dim=args.time_dim, history_size=args.history_size,
                         num_layers=args.num_layers, num_heads=args.num_heads,
                         dropout=args.dropout, use_cooccur=bool(args.use_cooccur)).to(device)
    ...
    if use_gev:
        ...
        if args.coupling == "score_ensemble":
            tgn_head = DyGFormerLiteLinkPredictor(args.embedding_dim).to(device)
    ...
    def stream_epoch(train: bool, do_val: bool, do_test: bool) -> Dict[str, float]:
        ...
        for (lo, hi) in batches:
            ...
            # representative query time for this batch (DyGFormer uses t for time encoding)
            ...
            if mode_train and len(local_train) > 0:
                ...
                k = len(ps) * 1  # 1 negative per positive
                neg = dst_pool[rng.integers(0, len(dst_pool), size=k)]
                allnodes = np.unique(np.concatenate([ps, pd, neg]))
                allnodes_t = torch.from_numpy(allnodes)
                t_q = torch.full((len(allnodes),), t_q_scalar, dtype=torch.long, device=device)
                z = tgn.embed(allnodes_t, t_q)
            ...
            for (local_idx, split, store) in eval_jobs:
                ...
                with torch.no_grad():
                    t_q = torch.full((len(allnodes),), int(ts.max()), dtype=torch.long, device=device)
                    z = tgn.embed(torch.from_numpy(allnodes), t_q)
                    idx_map = assoc_lookup(z, allnodes)
            ...
            # DyGFormer-lite has no memory state to detach
    ...
    model_name = f"DyGF+GEV({args.gev_indicators},{coup},pw={pw_mode})" if use_gev else "DyGF"
