    ...
    for run in range(args.num_runs):
        ...
        for epoch in range(args.num_epochs):
            ...
            if args.model_name == 'DyGFormer' and getattr(model[0], 'cohesion_cache', None) is not None:
                # reset GEV cache at the start of each epoch (chronological reset)
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name == 'DyGFormer' and getattr(model[0], 'cohesion_cache', None) is not None:
                    # advance the cohesion cache with this batch's (positive) edges
        ...
        # edges from earlier epochs leaking future structure into final eval).
        if args.model_name == 'DyGFormer' and getattr(model[0], 'cohesion_cache', None) is not None:
            _cache = model[0].cohesion_cache
            _cache.reset()
            _src = train_data.src_node_ids; _dst = train_data.dst_node_ids; _tt = train_data.node_interact_times
            for _i in range(0, len(_src), 1000):
                _cache.advance(_src[_i:_i + 1000], _dst[_i:_i + 1000], _tt[_i:_i + 1000])
