...
def get_link_prediction_tgb_data(dataset_name: str, max_edges: int = 0):
    """
    generate tgb data for link prediction task
    :param dataset_name: str, dataset name
    :param max_edges: int, if > 0, truncate to the first ``max_edges`` events (chronological).
        Used for broad-coverage sweeps on huge datasets where full-stream training is intractable.
    ...
    # --- optional truncation for broad-coverage sweeps -------------------------
    train_mask_full = dataset.train_mask
    val_mask_full = dataset.val_mask
    test_mask_full = dataset.test_mask
    if max_edges > 0 and max_edges < num_edges:
        print(f"[truncate] {dataset_name}: keeping the first {max_edges}/{num_edges} edges (chronological)")
        src_node_ids = src_node_ids[:max_edges]
        dst_node_ids = dst_node_ids[:max_edges]
        node_interact_times = node_interact_times[:max_edges]
        edge_ids = edge_ids[:max_edges]
        labels = labels[:max_edges]
        edge_raw_features = edge_raw_features[:max_edges]
        # truncate masks too — preserve TGB's chronological train/val/test cuts
        train_mask_full = train_mask_full[:max_edges]
        val_mask_full = val_mask_full[:max_edges]
        test_mask_full = test_mask_full[:max_edges]
        num_edges = max_edges
        # node count after truncation (subset of original)
        num_nodes = len(set(src_node_ids) | set(dst_node_ids))

    # Allow node-ids that are 0-based, 1-based, or bipartite-shifted (one side
    # may start at a larger offset, e.g. tgbl-wiki where dst is item ids that
    # follow user ids). We only require *some* side starts at 0 or 1.
    _src_min = int(src_node_ids.min()); _dst_min = int(dst_node_ids.min())
    assert _src_min <= 1 or _dst_min <= 1, "Node index should start from 0 or 1!"
    ...
    MAX_FEAT_DIM = 172
    # Size node_raw_features by *max node id* (not unique count) — several TGB
    # datasets have sparse node id ranges where max_id > num_unique. After +1
    # shift, ids run up to max_orig_id+1, so array must cover that.
    max_node_id = int(max(src_node_ids.max(), dst_node_ids.max()))
    if 'node_feat' not in data.keys():
        node_raw_features = np.zeros((max(num_nodes, max_node_id) + 1, 1))
    else:
        ...
        if node_raw_features.shape[0] <= max_node_id:
            extra = max_node_id + 1 - node_raw_features.shape[0]
            node_raw_features = np.vstack([node_raw_features,
                                           np.zeros((extra, node_raw_features.shape[1]), dtype=node_raw_features.dtype)])
