...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='tgbl-wiki',
                        choices=["tgbl-wiki", "tgbl-review", "tgbl-coin", "tgbl-comment", "tgbl-flight",
                                 "tgbl-uci", "tgbl-enron", "tgbl-subreddit", "tgbl-lastfm"])
    ...
    # GEV structure-channel swap-in (DyGFormer only)
    parser.add_argument('--structure_channel', type=str, default='cooccur',
                        choices=['none', 'cooccur', 'gev', 'both'],
                        help='DyGFormer structure channel: cooccur (Yu et al.), gev (cohesion-aware swap), both, or none')
    parser.add_argument('--gev_indicators', type=str, default='degree,core',
                        help='comma-separated indicators for the GEV cache (e.g. degree,core)')
