...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            # advance GEV cohesion cache (chronological) with this batch's positive edges, so
            # later eval queries see a graph state that includes earlier val/test edges.
            if model_name == 'DyGFormer' and getattr(model[0], 'cohesion_cache', None) is not None:
