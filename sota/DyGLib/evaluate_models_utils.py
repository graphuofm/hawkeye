...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   ...
                                   num_neighbors: int = 20, time_gap: int = 2000, advance_cohesion_cache: bool = True):
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            # advance GEV cache during eval (chronological). Only for the *main*
            # val/test stream — NOT for new-node subsets (which are re-evaluations
            # of a subset of already-streamed edges; advancing them double-counts).
            if advance_cohesion_cache and model_name == 'DyGFormer' and getattr(model[0], 'cohesion_cache', None) is not None:
