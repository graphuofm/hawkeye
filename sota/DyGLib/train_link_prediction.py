...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        _wf = getattr(args, 'gev_window_fraction', 0.0) or 0.0
        _sb = getattr(args, 'slot_backend', 'fast')
        _tag = f'struct{_struct}'
        if _struct in ('gev', 'both'):
            _tag += f'_wf{_wf}_{_sb}'
        args.save_model_name = f'{args.model_name}_{_tag}_seed{args.seed}'
        ...
        if args.model_name == 'TGAT':
            ...
            if getattr(args, 'structure_channel', 'cooccur') in ('gev', 'both'):
                ...
                # sliding-window: convert fraction -> absolute time span
                _gev_window_abs = 0.0
                _wf = float(getattr(args, 'gev_window_fraction', 0.0) or 0.0)
                if _wf > 0.0:
                    _all_t = full_data.node_interact_times
                    _gev_window_abs = _wf * (float(_all_t.max()) - float(_all_t.min()))
                    print(f"[gev] sliding window: fraction={_wf} -> window_abs={_gev_window_abs:.1f}")
                gev_cache = CohesionCache(
                    ...
                    use_csr=(getattr(args, 'slot_backend', 'fast') == 'full'),
                    device=None,
                )
            dynamic_backbone = DyGFormer(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                         ...
                                         slot_backend=getattr(args, 'slot_backend', 'fast'))
        else:
        ...
        for epoch in range(args.num_epochs):
            ...
            if args.model_name == 'DyGFormer' and getattr(model[0], 'cohesion_cache', None) is not None:
                model[0].cohesion_cache.reset()
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name == 'DyGFormer' and getattr(model[0], 'cohesion_cache', None) is not None:
                    model[0].cohesion_cache.advance(batch_src_node_ids, batch_dst_node_ids, batch_node_interact_times)
        ...
        # GEV cohesion cache: reset and replay the training stream so the cache
        # reflects exactly the post-training graph state (no leftover val/test
        # edges from earlier epochs — that would leak future structure).
        def _prime_cohesion_cache_with_train():
            _cache = getattr(model[0], 'cohesion_cache', None)
            if _cache is None:
                return
            _cache.reset()
            _bs = 1000
            _s, _d, _t = train_data.src_node_ids, train_data.node_interact_times, None
            _src = train_data.src_node_ids; _dst = train_data.dst_node_ids; _tt = train_data.node_interact_times
            for _i in range(0, len(_src), _bs):
                _cache.advance(_src[_i:_i + _bs], _dst[_i:_i + _bs], _tt[_i:_i + _bs])
        if args.model_name == 'DyGFormer':
            _prime_cohesion_cache_with_train()
