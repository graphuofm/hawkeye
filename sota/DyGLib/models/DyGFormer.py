...
class DyGFormer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 ...
                 structure_channel: str = 'cooccur', cohesion_cache=None, slot_backend: str = 'fast'):
        """
        DyGFormer model.
        ...
        # structure channel: 'cooccur' (original) | 'gev' (cohesion-aware swap) | 'both' | 'none'
        self.structure_channel = structure_channel
        self.cohesion_cache = cohesion_cache
        self.neighbor_co_occurrence_feat_dim = self.channel_embedding_dim
        if structure_channel in ('cooccur', 'both'):
        ...
        if structure_channel in ('gev', 'both'):
            assert cohesion_cache is not None, "structure_channel uses GEV but no cohesion_cache provided"
            self.cohesion_slot_encoder = CohesionSlotEncoder(
                cohesion_cache=cohesion_cache, feat_dim=self.neighbor_co_occurrence_feat_dim,
                device=self.device, backend=slot_backend)
        else:
            self.cohesion_slot_encoder = None
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
        ...
        # Structure channel — depending on structure_channel: cooccur (Yu et al.),
        # GEV (cohesion-aware swap-in), both, or neither.
        struct_feats_list_src, struct_feats_list_dst = [], []
        if self.neighbor_co_occurrence_encoder is not None:
            s_co, d_co = self.neighbor_co_occurrence_encoder(
                src_padded_nodes_neighbor_ids=src_padded_nodes_neighbor_ids,
                dst_padded_nodes_neighbor_ids=dst_padded_nodes_neighbor_ids)
            struct_feats_list_src.append(s_co); struct_feats_list_dst.append(d_co)
        if self.cohesion_slot_encoder is not None:
            s_gv, d_gv = self.cohesion_slot_encoder(
                src_node_ids=src_node_ids, dst_node_ids=dst_node_ids,
                src_padded_neighbor_ids=src_padded_nodes_neighbor_ids,
                dst_padded_neighbor_ids=dst_padded_nodes_neighbor_ids)
            struct_feats_list_src.append(s_gv); struct_feats_list_dst.append(d_gv)
        if struct_feats_list_src:
            # If 'both', sum the two channels (same feat_dim) so the downstream
            # projection layer stays unchanged.
            src_padded_nodes_neighbor_co_occurrence_features = torch.stack(struct_feats_list_src, dim=0).sum(0)
            dst_padded_nodes_neighbor_co_occurrence_features = torch.stack(struct_feats_list_dst, dim=0).sum(0)
        else:
            # 'none': zero channel; transformer just sees node/edge/time channels.
            B = len(src_node_ids)
            src_padded_nodes_neighbor_co_occurrence_features = torch.zeros(
                B, src_padded_nodes_neighbor_ids.shape[1], self.neighbor_co_occurrence_feat_dim, device=self.device)
            dst_padded_nodes_neighbor_co_occurrence_features = torch.zeros(
                B, dst_padded_nodes_neighbor_ids.shape[1], self.neighbor_co_occurrence_feat_dim, device=self.device)
...
# ---------------------------------------------------------------------- #
# GEV cohesion-aware structure channel (drop-in replacement for the
# NeighborCooccurrenceEncoder). Computed lazily per batch via CohesionCache.
# ---------------------------------------------------------------------- #
class CohesionSlotEncoder(nn.Module):
    """Cohesion-aware per-slot structure features. Same output shape as
    NeighborCooccurrenceEncoder so downstream layers stay untouched.

    ``backend`` controls feature richness:
      * ``fast`` (default): 6 dim, per-pair set intersection, O(avg_deg) per pair.
      * ``full``: ~23 dim from gev.features.pairwise (CSR matvec), slower but
        includes truss-weighted CN, 2-hop cohesive bridge, Adamic-Adar, etc.
        Use this on small datasets where compute is not a bottleneck.
    """

    def __init__(self, cohesion_cache, feat_dim: int, device: str = 'cpu', backend: str = 'fast'):
        super().__init__()
        self.cache = cohesion_cache
        self.feat_dim = feat_dim
        self.device = device
        self.backend = backend
        if backend == 'full':
            in_dim = int(cohesion_cache.pair_feat_dim)
        else:
            in_dim = int(cohesion_cache.FAST_SLOT_DIM)
        self.in_dim = in_dim
        self.encode_layer = nn.Sequential(
            nn.Linear(in_features=in_dim, out_features=feat_dim),
            nn.ReLU(),
    ...
    def forward(self,
                ...
                src_padded_neighbor_ids: np.ndarray,
                dst_padded_neighbor_ids: np.ndarray):
        """
        src_node_ids, dst_node_ids:        (B,) ndarray
        ...
        src_raw = src_raw * (1.0 - src_pad_mask)
        dst_raw = dst_raw * (1.0 - dst_pad_mask)
        src_raw = src_raw.to(self.device); dst_raw = dst_raw.to(self.device)
        return self.encode_layer(src_raw), self.encode_layer(dst_raw)
